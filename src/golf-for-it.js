...
const { Vector, vec, vec3, vec4, color, Mat4, Light, Scene, Texture } = tiny;
...
export class GolfForIt extends Scene {
    ...
    make_control_panel() {
        ...
        this.key_triggered_button(
            ...
            () => {
                if (
                    this.game_objects.golf_ball.rigidbody.velocity.norm() ===
                    0 &&
                    !this.is_game_done
                ) {
        ...
        // Reset control
        const reset_control = this.control_panel.appendChild(
            document.createElement("span")
        );
    ...
    display(context, program_state) {
        ...
        program_state.lights = [
            new Light(light_position, color(1, 1, 1, 1), 50000),
        ];
        ...
        if (!this.is_game_done) {
            if (
                test_collision(
                    this.game_objects.golf_ball,
                    this.game_objects.goal
                ) != null
            ) {
                this.end_game();
            }
    ...
    celebrate(dt) {
        ...
        goal.transform.position = vec3(
            goal.transform.position[0],
            goal.transform.position[1] + dt * 0.25,
            goal.transform.position[2]
        );
        goal.transform.rotation = vec3(
            goal.transform.rotation[0],
            goal.transform.rotation[1] + dt * 20,
            goal.transform.rotation[2]
        );
        if (goal.transform.size[0] > 0.01) {
            goal.transform.size = vec3(
                goal.transform.size[0] - dt * 0.2,
                goal.transform.size[0] - dt * 0.2,
                goal.transform.size[0] - dt * 0.2
            );
        } else {
